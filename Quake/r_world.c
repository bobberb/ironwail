...
static void R_InitBModelInstance (bmodel_gpu_instance_t *inst, entity_t *ent)
{
	...
	// For world entity, use appropriate default scale; otherwise use entity's scale
	unsigned char scale = ent == &cl_entities[0] ? (hexen2_mode ? 100 : ENTSCALE_DEFAULT) : ent->scale;
	R_EntityMatrix (mat, ent->origin, angles, scale);
